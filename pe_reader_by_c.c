...
int main(int argc, const char **argv[]) {
  ...
  int fpPosition =
      ftell(fp); // pe header를 읽는 데에 ms dos stub을 생략할 필요가 있기에, file offset을 pe header 바로 앞으로 e_lfanew를 이용해 바꿔주기 위한 변수
  ...
  IMAGE_SECTION_HEADER
      *section_header = (IMAGE_SECTION_HEADER *) malloc(sizeof(IMAGE_SECTION_HEADER) * number_of_sections);
  //section header 출력
  printf("\nSECTION HEADER\n");
  ...
  for (int i = 0; i < number_of_sections; i++) {
    ...
    fseek(fp, section_header[i].PointerToRawData, SEEK_SET);
    char *section = (char *) malloc(sizeof(char) * section_header[i].SizeOfRawData);
    fread(section, section_header[i].SizeOfRawData, 1, fp);

    for (int j = 0; j < section_header[i].SizeOfRawData; j++) {
      printf("%x ", (unsigned char)section[j]);
    }
    printf("\n\n");
    free(section);
  }
